...
int main(int argc, char *argv[])// на вход: частота дискретизации в кГц, разрешение, размер полученного файла в мб, все величины положительные
{ 
	srand(time(NULL));
	...
	{
		{"text", {{"text1",
	{ json::parse(u8R"("Производится четырёхканальная (квадро) звукозапись с частотой дискретизации ")"), "insert0", json::parse(u8R"(" кГц и ")"), "insert1", json::parse(u8R"("-битным разрешением. Результаты записи записываются в файл, сжатие данных не производится; размер полученного файла — ")"), "insert2", json::parse(u8R"(" Мбайт. Определите приблизительно время записи (в секундах), ответ округлите до большего.")")}}},
